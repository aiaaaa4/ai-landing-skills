...
- 字幕翻译始终运行 Fun-ASR 获取词级时间戳。优先复用 `.work/input/` 音频；原语言字幕仅用于校正 ASR 显示内容和翻译输入，不能替代 ASR 时间轴。`qwen-mt-plus` 只负责初译，编排模型必须通读全部章节完成语义重分段，并在确定性 QA 后完成最终全文一致性 QC；两道全局门禁通过后才导出并清理 `.work/input/`。
