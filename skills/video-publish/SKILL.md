...
默认流程是：从视频前半段抽取 `5` 张封面候选，等待用户选择；显示选定封面 `1.5` 秒，再显示固定免责声明 `2` 秒，最后无损拼接原视频。只编码短片头，原视频主体使用码流复制。
...
2. **片头时长**：封面默认 `1.5` 秒；免责声明默认使用 `assets/disclaimer-zh-en-1920x1080.png` 并显示 `2` 秒。
...
使用 `--preview-content-seconds 8` 可先输出约 `11.5` 秒预览。默认只支持将短片头无损拼接到 H.264 `yuv420p` + AAC MP4；其他编码格式应明确告知用户并改用完整重编码。
