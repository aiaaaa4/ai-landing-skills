...
5. **专业封装选项**：是否裁去开头/结尾、是否保留原分辨率、画质优先或速度优先、是否需要网页快速起播，以及是否限制最终文件体积。默认是保留原分辨率、速度与画质平衡、启用 `faststart`、不裁切。
...
- `--max-size-multiplier 3`：按源文件时长自动分配视频和音频码率，限制最终 MP4 不超过源文件体积的三倍；超出时会删除该次输出并报错。
