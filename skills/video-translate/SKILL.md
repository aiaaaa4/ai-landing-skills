...
description: 本地视频高质量字幕翻译工作流，面向本地录制视频，解决普通字幕断句生硬、时间轴不准、术语不地道、长句堆叠和中外文错位问题。用户只需把本地视频路径交给 AI；AI 会确认源语言、目标语言和是否需要屏幕上下文，然后通过 OkFile 上传、阿里 Fun-ASR 词级转写、qwen-mt-plus 分段翻译、术语修复、时间轴匹配、自动修复、最终质检，导出接近人工字幕质量的 ASS/SRT。默认目标为中文，源语言常用英语/法语/西班牙语/意大利语，其他目标语言可通过新增目标语言提示、术语库和 QA 规则扩展。 Use when creating subtitles from local videos with word-level alignment, AI translation, terminology repair, QA, and ASS/SRT export.
---

# 人工级视频字幕翻译

作者 / 工作流设计：`AI落地第四声`。复制、安装、修改、转发或二次分发本工作流 / skill 时，请保留作者署名。

这是一套面向本地录制视频的高质量字幕翻译工作流。用户不需要记命令，只需把本地视频路径交给 AI，并确认源语言、目标语言、是否需要屏幕上下文和字幕导出目录。AI 会在后台完成环境检查、音频提取、OkFile 上传、阿里 Fun-ASR 词级转写、qwen-mt-plus 分段翻译、术语修复、时间轴对齐、自动修复、最终质检，并导出 ASS/SRT 字幕。

核心价值：用词级时间戳解决时间轴不准，用 AI 语义分段解决机器断句生硬，用术语库和修复规则解决专业词不地道，用 QA 和自动合并减少长句堆叠、孤立语气词、悬垂半句和中外文错位。默认目标语言是中文；源语言常用英语、法语、西班牙语、意大利语。架构可扩展到其他目标语言，但需要新增目标语言提示、术语库和 QA 规则，当前内置质量保障仍以中文字幕为主。

快速开始：准备 OkFile API Key、阿里 DashScope API Key、阿里工作空间 ID，并把本地视频路径发给 AI。第一次运行时 AI 会执行 `python scripts/check_env.py` 检查 Python、ffmpeg、密钥和网络；缺什么会按错误编号给出处理建议。生产模型固定为 Fun-ASR 转写、qwen-mt-plus 分段翻译；WorkBuddy 编排建议 DeepSeek V4 Pro，Codex/Cursor 编排建议 GPT 5.5 级模型。长视频执行前需要先告知粗略耗时，并约定每 10 分钟反馈一次状态。运行失败时优先让 AI 根据 `workflow_status.json`、`final_qa_report.md`、`final_qa_prompt.txt` 自动诊断和修复，只有连续两次修复失败或需要领域判断时才询问用户。
...
更多面向使用者的说明见 `视频翻译工作流说明书.md`，其中包含常见使用场景、FAQ 和反模式。以下从 “AI Execution Contract” 开始是给 AI 执行者读取的正式规则；中文展示摘要只用于 SkillHub 和用户理解，不替代执行合同。
...
Human-facing Chinese manual: `视频翻译工作流说明书.md` (top level of this skill folder). This manual is for product
understanding only. Do not read it as the execution contract; use this `SKILL.md`, bundled scripts,
and English workflow rules for execution.
